...
/// Removal of items is performed via the `onRemove` closure.
struct ShopCartSummary: View {
    let items: [Product]
    let onCheckout: () -> Void
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if expanded {
                ...
                VStack(spacing: 6) {
                    ForEach(itemCounts.indices, id: \.self) { i in
                        let entry = itemCounts[i]
                        HStack {
            ...
            Button(action: onCheckout) {
                HStack(spacing: 8) {
                    Text("Verify Availability — \(String(format: "$%.2f", total))")
                        .font(.system(size: 15, weight: .semibold))
                }
